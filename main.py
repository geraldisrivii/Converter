...
with open("paths.json") as programm_file:
    ...
    for index in range(len(initial_names)):
        if (key == "p"):
            ...
            if(not (index1 == -1)):
                print(programm_data["path_output"] + new_names[index], (programm_data["path_output"] + initial_names[index][:index1] +
                initial_names[index][index1 + 1:initial_names[index].find(".")] + programm_data["format_output"]).strip(), sep="\t")
        else:
            index1 = initial_names[index].find(" ")
            if(not (index1 == -1)):
